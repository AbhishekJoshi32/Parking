...
{
	$pass=$_POST['pass'];
	$res=mysql_query("SELECT password from customer_det where username='".$_SESSION['user_name']."'");
	if(!$res)
	{
	...
	else
	{
		while($row=mysql_fetch_array($res))
		{
			if($pass=$row['password'])
			{
			   ...
			   {
				$sql1="UPDATE customer_det SET password='".sha1($_POST['new_pass'])."' WHERE username='".$_SESSION['user_name']."'";
